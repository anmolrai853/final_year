...
class _PlannerPageState extends State<PlannerPage>
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildStatCard('Sessions', '${sessions.length}',
                    Icons.menu_book, const Color(0xFF3B82F6)),
              ),
          ...
          if (sessions.isEmpty)
            _buildEmptyState(
                'No study sessions today', 'Tap the + button to add a session')
          else
        ...
        children: [
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 6,
                  decoration: BoxDecoration(
                      color: d.urgencyColor, shape: BoxShape.circle),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(d.title,
                ...
                Text(d.timeRemainingText,
                    style: TextStyle(
                        color: d.urgencyColor, fontSize: 11)),
              ],
            ),
          )),
          if (deadlines.length > 3)
            Padding(
              ...
              child: Text('+${deadlines.length - 3} more',
                  style: const TextStyle(
  ...
  Widget _buildWeekTab() {
    ...
    for (var i = 0; i < 7; i++) {
      sessionsByDay[weekStart.add(Duration(days: i))] = [];
    }
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildWeekStat('Completed',
                    '${sessions.where((s) => s.isCompleted).length}'),
                _buildWeekStat(
                    'Total Hours',
                    '${(sessions.fold<int>(0, (sum, s) => sum + (s.actualDurationMinutes ?? s.durationMinutes)) / 60).toStringAsFixed(1)}'),
              ],
            ),
          ),
          ...
          children: [
            ...
            const Text('No Deadlines Yet',
                style: TextStyle(
                    ...
                    color: Colors.white)),
            const SizedBox(height: 8),
            Text('Add your coursework deadlines\nto stay on top of things',
        ...
        children: [
          ...
          if (overdue.isNotEmpty) ...[
            _buildDeadlineSectionHeader(
                'Overdue', overdue.length, const Color(0xFFEF4444)),
            const SizedBox(height: 8),
          ...
          if (upcoming.isNotEmpty) ...[
            _buildDeadlineSectionHeader(
                'Upcoming', upcoming.length, const Color(0xFF3B82F6)),
            const SizedBox(height: 8),
          ...
          if (completed.isNotEmpty) ...[
            _buildDeadlineSectionHeader(
                'Completed', completed.length, const Color(0xFF10B981)),
            const SizedBox(height: 8),
            ...completed.take(5).map((d) => _buildDeadlineCard(d)),
            if (completed.length > 5)
              Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Text('+${completed.length - 5} more completed',
                    style: const TextStyle(
  ...
  Widget _buildDeadlineStats(
      List<Deadline> active, List<Deadline> completed) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          _buildDeadlineStatItem(
              'Active', '${active.length}', const Color(0xFF3B82F6)),
          _buildDeadlineStatItem(
              'This Week', '$dueThisWeek', const Color(0xFFF59E0B)),
          _buildDeadlineStatItem(
              'Overdue', '$overdue', const Color(0xFFEF4444)),
          _buildDeadlineStatItem(
              'Done', '${completed.length}', const Color(0xFF10B981)),
        ],
      ),
    );
  }
  ...
  Widget _buildDeadlineCard(Deadline deadline) {
    ...
    final loggedHours =
    _controller.getLoggedHoursForDeadline(deadline.id);
          ...
          children: [
            Row(
              children: [
                GestureDetector(
                  onTap: () =>
                      _controller.toggleDeadlineStatus(deadline.id),
                  child: Container(
                    width: 28,
                    height: 28,
                    decoration: BoxDecoration(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color:
                        deadline.priority.color.withOpacity(0.4)),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(deadline.priority.icon,
                          size: 12, color: deadline.priority.color),
                      const SizedBox(width: 4),
            ...
            Row(
              children: [
                Icon(Icons.schedule,
                    size: 14, color: deadline.urgencyColor),
                const SizedBox(width: 4),
                Text(
                  '${dateFormat.format(deadline.dueDate)} at ${timeFormat.format(deadline.dueDate)}',
                  style: TextStyle(
                      fontSize: 12, color: deadline.urgencyColor),
                ),
                const Spacer(),
                if (deadline.moduleCode != null)
                  Container(
                    ...
                    child: Text(deadline.moduleCode!,
                        style: TextStyle(
                            fontSize: 10,
                            color: moduleColor,
                            fontWeight: FontWeight.w600)),
                ...
                children: [
                  ...
                  Expanded(
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(4),
                      child: LinearProgressIndicator(
                        value: (loggedHours / deadline.estimatedHours)
                            .clamp(0.0, 1.0),
                        backgroundColor: const Color(0xFF1E293B),
            ...
            if (deadline.status != DeadlineStatus.completed) ...[
              ...
              Text(deadline.timeRemainingText,
                  style: TextStyle(
                      fontSize: 11,
  ...
  Widget _buildSessionCard(StudySession session) {
    ...
    return GestureDetector(
      ...
      // Long press triggers completion flow
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Row(
              children: [
                ...
                Container(
                  ...
                  child: Icon(session.type.icon, color: color, size: 20),
                ),
                const SizedBox(width: 12),
                ...
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    // Pomodoro button — always shown for incomplete sessions
                    ...
                    // Completion toggle — always visible regardless of isPast
                    if (session.isCompleted)
                      GestureDetector(
                        onTap: () => _controller
                      ...
                      GestureDetector(
                        onTap: () => _showCompletionDialog(session),
                        child: Container(
                          padding: const EdgeInsets.symmetric(
            ...
            // Long press hint for incomplete sessions
            if (!session.isCompleted) ...[
              const SizedBox(height: 4),
              Row(
                children: [
                  ...
                  Icon(Icons.touch_app,
                      size: 10, color: Colors.white.withOpacity(0.18)),
  ...
  void _handleSessionLongPress(StudySession session) {
    if (!session.isCompleted) {
      _showCompletionDialog(session);
    } else {
      _controller.toggleSessionCompletion(session.id);
  ...
  DateTime _getWeekStart(DateTime date) {
    ...
    return DateTime(
        date.year, date.month, date.day - (weekday - 1));
  }
}